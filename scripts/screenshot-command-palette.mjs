#!/usr/bin/env node
// One-off check for the Cmd+K command palette.
...
await page.screenshot({ path: path.join(outDir, "command-palette-open.png") });

// Type to filter.
await page.keyboard.type("deep");
await page.waitForTimeout(150);
await page.screenshot({ path: path.join(outDir, "command-palette-filtered.png") });

console.log("Saved command palette screenshots");
await browser.close();
server.kill();
process.exit(0);
